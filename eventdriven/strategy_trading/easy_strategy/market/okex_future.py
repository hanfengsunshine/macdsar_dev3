...
class OkexFutureMarket(MarketBase):
    def __init__(self, url):
        super(OkexFutureMarket, self).__init__()
        self._url = url
        self._id = int(time.time() * 1000000)
        self._subbed_topics = set()
        self._ws = None
        self._instrument_mngr = InstrumentManager()
        self._exchange = 'OKEX_CONTRACT'
    ...
    async def _run_ws(self):
        ...
        logger.info('okex future market connected')
        ...
        try:
            ...
            logger.warning('okex future market disconnectd. error={}', self._url, e)
            await self._reconnect()
            return
    ...
    async def _subscribe_topic(self, topic):
        ...
        if self._ws:
            logger.info('[okex][future][s] {}', request)
            try:
    ...
    async def _on_ws_message(self, message: str):
        ...
        if 'event' in message:
            logger.info('[okex][future][r] {}', message)
            return
