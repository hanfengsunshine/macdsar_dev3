...
class HuobiLinearSwapMarket(MarketBase):
    def __init__(self, url):
        super(HuobiLinearSwapMarket, self).__init__()
        self._url = url
        self._id = int(time.time() * 1000000)
        self._subbed_topics = set()
        self._ws = None
        self._instrument_mngr = InstrumentManager()
        self._exchange = 'HUOBI_SWAP_USDT'
        self._heartbeat = 0
    ...
    async def _run_ws(self):
        ...
        logger.info('huobi linear-swap market connected')
        ...
        try:
            ...
            logger.warning('huobi linear-swap market disconnectd. error={}', e)
    ...
    async def _check_heartbeat(self):
        ...
        if self._heartbeat != 0 and current_sec - self._heartbeat > self._timeout_sec:
            self._heartbeat = 0
            logger.warning('[huobi][linear-swap] heartbeat lost')
            await self._ws.close()
        asyncio.ensure_future(self._check_heartbeat())
    ...
    async def _subscribe_topic(self, topic):
        ...
        if self._ws:
            logger.info('[huobi][linear-swap][s] {}', request)
            try:
    ...
    async def _on_ws_message(self, message: str):
        ...
        if 'ch' not in message:
            if 'ping' in message:
                ...
                logger.info('[huobi][linear-swap][r] {}', message)
            return
        ch = message['ch']
        if 'bbo' in ch:
            symbol = ch.split('.')[1]
