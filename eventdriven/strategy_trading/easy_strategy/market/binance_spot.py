...
class BinanceSpotMarket(MarketBase):
    def __init__(self, url):
        super(BinanceSpotMarket, self).__init__()
        self._url = url
        self._id = int(time.time() * 1000000)
        self._subbed_topics = set()
        self._ws = None
        self._instrument_mngr = InstrumentManager()
        self._exchange = 'BINANCE_SPOT'
    ...
    async def subscribe(self,
        ...
        if trade:
            topic = '{}@trade'.format(instrument.exchange_symbol.lower())
            await self._subscribe_topic(topic)
        if top_orderbook:
    ...
    async def _run_ws(self):
        ...
        logger.info('binance spot market connected')
        ...
        try:
            ...
            logger.warning('binance spot market disconnectd. error={}', e)
            await self._reconnect()
            return
    ...
    async def _subscribe_topic(self, topic):
        while True:
            ...
            if current - self._last_request_timestamp > 0:
                break
            await asyncio.sleep(1)
        ...
        if self._ws:
            logger.info('[binance][spot][s] {}', request)
    ...
    async def _on_ws_message(self, message: str):
        received_timestamp = int(time.time() * 1000000)
        message = orjson.loads(message)
        if 'stream' not in message:
            logger.info('[binance][spot][r] {}', message)
            return
        ...
        if 'bookTicker' in stream:
            ...
            if 'depth@' not in stream:
                ...
                if instrument:
                    ...
                    fixed_ob.on_full_snapshot(data['bids'], data['asks'], received_timestamp, int(time.time() * 1000000), data['lastUpdateId'])
            ...
            if instrument:
                ...
                trades.append(Trade(
                    ...
                    exchange_timestamp=data['E'] * 1000000,
                    connectivity_timestamp=int(time.time() * 1000000)
                ))
