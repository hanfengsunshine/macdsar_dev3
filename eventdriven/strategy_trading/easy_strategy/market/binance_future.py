...
class BinanceFutureMarket(MarketBase):
    def __init__(self, url):
        super(BinanceFutureMarket, self).__init__()
        self._url = url
        self._id = int(time.time() * 1000000)
        self._subbed_topics = set()
        self._ws = None
        self._instrument_mngr = InstrumentManager()
        self._exchange = 'BINANCE_CONTRACT'
    ...
    async def _run_ws(self):
        ...
        logger.info('binance future market connected')
        ...
        try:
            ...
            logger.warning('binance future market disconnectd. error={}', e)
            await self._reconnect()
            return
    ...
    async def _subscribe_topic(self, topic):
        ...
        if self._ws:
            logger.info('[binance][future][s] {}', request)
            try:
    ...
    async def _on_ws_message(self, message: str):
        ...
        if 'stream' not in message:
            logger.info('[binance][future][r] {}', message)
            return
