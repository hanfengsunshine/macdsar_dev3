...
class BinanceSwapMarket(MarketBase):
    def __init__(self, url):
        super(BinanceSwapMarket, self).__init__()
        self._url = url
        self._id = int(time.time() * 1000000)
        self._subbed_topics = set()
        self._ws = None
        self._instrument_mngr = InstrumentManager()
        self._exchange = 'BINANCE_SWAP'
    ...
    async def _run_ws(self):
        ...
        logger.info('binance swap market connected')
        ...
        try:
            ...
            logger.warning('binance swap market disconnectd. error={}', e)
            await self._reconnect()
            return
    ...
    async def _subscribe_topic(self, topic):
        ...
        if self._ws:
            logger.info('[binance][swap][s] {}', request)
            try:
    ...
    async def _on_ws_message(self, message: str):
        ...
        if 'stream' not in message:
            logger.info('[binance][swap][r] {}', message)
            return
